...
Boots directly and exclusively into FORGE (the holographic 3-D modeller).
...
def _beep(freq=800, duration=0.08, volume=0.12):
    try:
...
def main():
    ...
    print('  Left hand  = VIOLET   (orbit / Z-depth)')
    print('  Right hand = CYAN     (select / orbit / XY)')
    print('  Both hands = scale / spin / drag scene')
    ...
    # Boot straight into FORGE — the only app
    ...
    try:
        while True:
            ...
            if not still_open:
                # Re-launch FORGE immediately — it's the only app
                active_app = Modeller3D()
                _beep(600, 0.10, 0.10)
            ...
            cv2.imshow('STARK FORGE  //  DUAL-HAND', frame)
